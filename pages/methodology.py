...
layout = html.Div([
    ...
    dcc.Graph(figure=data.permutation_importance.visualize()),
    ...
    dcc.Graph(figure=data.cross_sectional_regression.visualize()),
    html.H4("What it means:"),
    ...
    html.H3("Quantile Regression Analysis"),
    dcc.Tabs([
        dcc.Tab(label='Wage Growth in Dollars', children=[
            dcc.Graph(figure=data.quantile.visualize(percentage=False))
        ]),
        dcc.Tab(label='Wage Growth Percentage', children=[
            dcc.Graph(figure=data.quantile.visualize(percentage=True))
        ])
    ]),
    html.H4("What it means:"),
    html.P("These plots show the quantile wage growths and growth percentages for each class. This effectively allows us to compare wage growth at each percentile for each class, which helps us determine whether there is a trend between income level, and income class to wage growth."),
    html.H4("What it shows:"),
